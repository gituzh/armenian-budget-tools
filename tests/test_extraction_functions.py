...
# Add the src directory to Python path to import budget module
budget_module_path = Path(__file__).parent.parent / "src" / "budget-am"
sys.path.insert(0, str(budget_module_path))
from budget import (
...
class TestErrorHandling:
    ...
    def test_malformed_excel_data(self):
        ...
        with tempfile.NamedTemporaryFile(suffix=".xlsx", delete=False) as tmp:
            df = pd.DataFrame({
                "A": ["random", "data", "without"],
                "B": ["proper", "budget", "structure"]
            })
            df.to_excel(tmp.name, index=False, header=False)
            tmp_path = tmp.name
