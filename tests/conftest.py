        ...
        if year_dir.is_dir() and year_dir.name.isdigit():
            ...
            for source_type_dir in year_dir.glob("*"):
                if source_type_dir.is_dir():
                    ...
                    overall_path = source_type_dir / f"{year}_{source_type}_overall.json"
                    
                    if csv_path.exists() and overall_path.exists():
                        if source_type in data_by_type:
...
def get_all_available_data() -> List[Tuple[int, str]]:
    """Get all available budget data (year, source_type) combinations."""
    output_dir = Path(__file__).parent.parent / "output"
    data_combinations = []
...
def load_budget_data(year: int, source_type: str) -> BudgetDataInfo:
    """Load budget data for a specific year and source type."""
    base_dir = Path(__file__).parent.parent
    file_path = base_dir / f"output/{year}/{source_type}/{year}_{source_type}.csv"
    overall_path = base_dir / f"output/{year}/{source_type}/{year}_{source_type}_overall.json"
...
def get_financial_columns(source_type: str) -> Dict[str, List[str]]:
    ...
    if source_type == "BUDGET_LAW":
        ...
        base_cols = [
            "annual_plan", "rev_annual_plan", "period_plan", 
            "rev_period_plan", "actual"
        ]
        return {
