...
def get_available_budget_data():
    """Get all available budget data (year, source_type) combinations from the output directory."""
    output_dir = Path("output")
    data_combinations = []
    for year_dir in output_dir.glob("*"):
        if year_dir.is_dir() and year_dir.name.isdigit():
            year = int(year_dir.name)
            for source_type_dir in year_dir.glob("*"):
                if source_type_dir.is_dir():
                    source_type = source_type_dir.name
                    csv_path = source_type_dir / f"{year}_{source_type}.csv"
                    overall_path = (
                        source_type_dir / f"{year}_{source_type}_overall.json"
                    )
                    if csv_path.exists() and overall_path.exists():
                        data_combinations.append((year, source_type))
    return sorted(data_combinations)


@pytest.fixture(params=get_available_budget_data())
def budget_data(request):
    """
    Parametrized fixture that loads budget data for each available (year, source_type) combination.
    Returns a tuple of (year, source_type, DataFrame, overall_values).
    """
    year, source_type = request.param
    file_path = f"output/{year}/{source_type}/{year}_{source_type}.csv"
    overall_path = (
        f"output/{year}/{source_type}/{year}_{source_type}_overall.json"
    )

    df = pd.read_csv(file_path, encoding="utf-8-sig")

    # Load overall values from JSON
    with open(overall_path, "r", encoding="utf-8") as f:
        overall_values = json.load(f)

    return year, source_type, df, overall_values
...
def test_subprogram_sums_match_program_totals(budget_data):
    ...
    assert len(mismatches) == 0, (
        f"{year}/{source_type}: Found mismatches between subprogram sums and program totals:\n"
...
def test_program_distribution(budget_data):
    ...
    ), f"{year}/{source_type}: All state bodies have the same number of program codes"
    assert (
        program_counts.max() > 1
    ), f"{year}/{source_type}: No state body has multiple programs"
...
def test_grand_total_consistency(budget_data):
    ...
    # Extract grand total from overall values
    # Handle both formats: dict with "overall_total" key or direct value for 2025
    if isinstance(overall_values, dict):
        if "overall_total" in overall_values:
            grand_total = overall_values["overall_total"]
        else:
            # For spending reports, use the main amount column (usually overall_actual)
            # Find the first numeric value that looks like a total
            for key, value in overall_values.items():
                ...
                # If no obvious total found, use overall_actual as fallback
                grand_total = overall_values.get("overall_actual", 0)
    else:
    ...
    if grand_total != state_body_sum:
        error_msg = f"{year}/{source_type}: Grand total ({grand_total}) differs from state body totals ({state_body_sum}) by {grand_total - state_body_sum}"
        raise AssertionError(error_msg)

    if grand_total != program_sum:
        error_msg = f"{year}/{source_type}: Grand total ({grand_total}) differs from program totals ({program_sum}) by {grand_total - program_sum}"
        raise AssertionError(error_msg)

    if grand_total != subprogram_sum:
        error_msg = f"{year}/{source_type}: Grand total ({grand_total}) differs from subprogram totals ({subprogram_sum}) by {grand_total - subprogram_sum}"
        raise AssertionError(error_msg)
...
def test_state_body_total_consistency(budget_data):
    ...
    if mismatches_program or mismatches_subprogram:
        error_msg = f"{year}/{source_type}: Found mismatches in {len(mismatches_program)} state bodies for program totals and {len(mismatches_subprogram)} for subprogram totals"
...
def test_program_total_consistency(budget_data):
    ...
    if len(mismatches) > 0:
        ...
        error_msg = (
            f"{year}/{source_type}: Found {len(mismatches)} programs with total difference of {total_difference}"
...
def test_data_quality(budget_data):
    ...
    for column in required_columns:
        ...
        assert (
            null_count == 0
        ), f"{year}/{source_type}: Found {null_count} null values in column '{column}'"
        ...
        if df[column].dtype == "object":
            ...
            assert (
                empty_count == 0
            ), f"{year}/{source_type}: Found {empty_count} empty strings in column '{column}'"
        ...
        if column in ["state_body_total", "program_total", "subprogram_total"]:
            ...
            if not negative_rows.empty:
                warning_msg = (
                    f"\n{year}/{source_type}: Found {len(negative_rows)} negative values in column '{column}':"
                    f"\nNegative rows:"
...
def test_program_codes_format(budget_data):
    ...
    assert df["program_code"].dtype in [
        "int64",
        "int32",
    ], f"{year}/{source_type}: program_code should be integer type, found {df['program_code'].dtype}"

    # Check subprogram_code is integer
    assert df["subprogram_code"].dtype in [
        "int64",
        "int32",
    ], f"{year}/{source_type}: subprogram_code should be integer type, found {df['subprogram_code'].dtype}"

    # For 2025, verify program_code_ext
    if year == 2025:
        assert (
            "program_code_ext" in df.columns
        ), f"{year}/{source_type}: program_code_ext column is missing"

        assert df["program_code_ext"].dtype in [
            "int64",
            "int32",
        ], f"{year}/{source_type}: program_code_ext should be integer type, found {df['program_code_ext'].dtype}"

        # Verify program_code matches program_code_ext
        mismatches = df[df["program_code"] != df["program_code_ext"]]
        assert len(mismatches) == 0, (
            f"{year}/{source_type}: Found {len(mismatches)} rows where program_code doesn't match program_code_ext:\n"
